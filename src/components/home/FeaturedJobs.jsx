...
import { FiMapPin, FiDollarSign, FiArrowRight, FiArrowLeft, FiClock, FiBookmark } from 'react-icons/fi';
      ...
      <div className="container mx-auto px-4 sm:px-6 mb-12">
        <div className="flex flex-col md:flex-row justify-between items-end gap-6">
          <div className="max-w-2xl">
            ...
            >
              <h2 className="text-4xl md:text-5xl font-extrabold mb-4 tracking-tight">
                Top <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-400">Featured</span> Jobs
              </h2>
              <p className="text-zinc-400 text-lg">
                Discover elite opportunities hand-picked just for you. Apply to top-tier companies worldwide.
              </p>
            </motion.div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <Link href="/jobs" className="group flex items-center gap-2 px-6 py-3 rounded-full bg-white/5 border border-white/10 hover:bg-white/10 text-white font-medium transition-all">
        ...
        >
          {loading ? (
            ...
            Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="min-w-[340px] md:min-w-[400px] h-[320px] bg-zinc-900/40 border border-white/5 rounded-3xl animate-pulse shrink-0 snap-center"></div>
            ))
          ) : (
            jobs.map((job, index) => (
              <motion.div 
                ...
                <div className="bg-[#09090b] border border-white/10 hover:border-blue-500/30 rounded-[2rem] p-7 h-full flex flex-col justify-between transition-all duration-500 hover:shadow-[0_0_40px_-15px_rgba(37,99,235,0.3)] hover:-translate-y-1.5 relative overflow-hidden z-10">
                  ...
                  <div className="relative z-20">
                    <div className="flex justify-between items-start mb-6">
                      <div className="w-14 h-14 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center p-3 shadow-inner group-hover:scale-105 group-hover:border-blue-500/30 group-hover:bg-blue-500/5 transition-all duration-300 shrink-0">
                        {job.companyLogo ? (
                          <Image src={job.companyLogo} alt={job.companyName || 'Company'} width={40} height={40} className="object-contain drop-shadow-md" />
                        ) : (
                    ...
                    <p className="text-zinc-400 text-[13px] font-semibold tracking-wide uppercase mb-6">{job.companyName}</p>
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-8">
                      <div className="flex items-center gap-2.5 bg-white/5 border border-white/5 px-3.5 py-2.5 rounded-xl text-[11px] font-semibold text-zinc-300 group-hover:bg-white/10 transition-colors uppercase tracking-wider">
                          ...
                          <span className="truncate">{job.jobType?.replace('-', ' ')}</span>
                      </div>
                      <div className="flex items-center gap-2.5 bg-white/5 border border-white/5 px-3.5 py-2.5 rounded-xl text-[11px] font-semibold text-zinc-300 group-hover:bg-white/10 transition-colors uppercase tracking-wider">
                          <FiMapPin className="text-blue-400/70 w-3.5 h-3.5" />
                          <span className="truncate">{job.isRemote ? 'Remote' : (job.location || 'Remote')}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="relative z-20 flex items-center justify-between mt-auto pt-5 border-t border-white/5">
                    <div className="flex flex-col">
                      <span className="text-[11px] text-zinc-500 uppercase tracking-wider font-semibold mb-1">Salary</span>
                    ...
                    <Link href={`/jobs/${job._id}`} className="w-10 h-10 rounded-full bg-white/10 group-hover:bg-blue-600 flex items-center justify-center text-white transition-colors duration-300 transform group-hover:scale-110 shrink-0">
