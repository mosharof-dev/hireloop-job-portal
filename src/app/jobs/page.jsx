...
const GetAllJobs = async ({ searchParams }) => {
  let jobs = null;
  let error = null;
  ...
  console.log("server side q", resolvedSearchParams);

  try {
    jobs = await getAllJobs(resolvedSearchParams);
    console.log(`Filtered Jobs Length: ${jobs?.length || 0}`);
  } catch (err) {
    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="mb-8">
        ...
        <p className="text-zinc-400">
          {!isError
            ? `Find your dream job from our available positions`
